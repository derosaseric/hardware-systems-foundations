...
struct PacketHeader parse_header(const uint8_t *packet)
{
    struct PacketHeader header;

    header.message_type = packet[0];
    header.flags = packet[1];
    header.payload_length = read_u16_be(&packet[2]);
    header.sequence_number = read_u32_be(&packet[4]);
...
int main(void)
{
    ...
    struct PacketHeader header = parse_header(packet);
